...
-- Таблица прикрепленных к посту медиа-файлов
...
CREATE TABLE media_posts (
	...
	order_val INT UNSIGNED NOT NULL COMMENT "Порядковый номер медиа файла в рамках поста", 
	created_at DATETIME DEFAULT NOW() COMMENT "Время создания строки"
) COMMENT "Прикрепленные к посту медиа файлы";
...
-- Таблица лайков, выставляемых для пользователей
DROP TABLE IF EXISTS likes_users;
CREATE TABLE likes_users (
  from_user_id INT UNSIGNED NOT NULL COMMENT "Ссылка на пользователя, поставившего лайк",
...
-- Таблица лайков, выставляемых для медиафайлов
DROP TABLE IF EXISTS likes_media;
CREATE TABLE likes_media (
  from_user_id INT UNSIGNED NOT NULL COMMENT "Ссылка на пользователя, поставившего лайк",
  to_media_id INT UNSIGNED NOT NULL COMMENT "Ссылка на медиафайл, которому поставили лайк",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",  
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки",
  PRIMARY KEY (from_user_id, to_media_id) COMMENT "Составной первичный ключ"
) COMMENT "Лайки для медиа файлов";

-- Таблица лайков, выставляемых для постов
DROP TABLE IF EXISTS likes_posts;
CREATE TABLE likes_posts (
  from_user_id INT UNSIGNED NOT NULL COMMENT "Ссылка на пользователя, поставившего лайк",
  to_post_id INT UNSIGNED NOT NULL COMMENT "Ссылка на пост, которому поставили лайк",
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT "Время создания строки",  
  updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT "Время обновления строки",
